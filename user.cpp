...
User::User(std::string user, std::string pwd_hash, std::string name, std::string email, Role r, bool isAutoPwd)
    : username(user), password_hash(pwd_hash), full_name(name), email(email), role(r), isAutoGeneratedPwd(isAutoPwd) {}

std::string User::serialize() const {
    std::ostringstream oss;
    oss << username << "|" << password_hash << "|" << full_name << "|" << email << "|"
